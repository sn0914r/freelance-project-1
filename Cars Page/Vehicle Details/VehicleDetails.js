...
function renderVehicleDetails() {
  ...
  if (!vehicle) {
    document.querySelector('.vehicle-info').innerHTML = '<h2 class="text-danger">Vehicle not found</h2>';
    return;
  }
  ...
  if (vehicle.images && vehicle.images.length > 0) {
    thumbnailContainer.innerHTML = vehicle.images.map((img, idx) =>
      `<img src="../assets/${img}" alt="${vehicle.name} ${idx+1}" class="thumbnail${idx === 0 ? ' active' : ''}" onclick="changeImage(this)">`
    ).join('');
  }
  ...
  document.querySelector('.discounted-price').textContent = `₹${vehicle.discountedPrice}`;

  // Rental details
  const detailValues = document.querySelectorAll('.details-section .detail-value');
  if (detailValues.length >= 4) {
    detailValues[0].textContent = `₹${vehicle.discountedPrice}`;
    detailValues[1].textContent = 'Yes';
    detailValues[2].textContent = `₹${Math.round(vehicle.discountedPrice/24)}`;
    detailValues[3].textContent = 'Unlimited';
  }
}
